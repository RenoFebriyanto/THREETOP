    ...
    return (
      <div className="w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm">Masukkan email untuk menerima link reset password</p>
        </div>

        <div className="rounded-2xl p-8 border border-slate-700/50 shadow-2xl"
          style={{ background: 'linear-gradient(135deg, rgba(15,23,42,0.95) 0%, rgba(15,20,35,0.98) 100%)', backdropFilter: 'blur(20px)' }}>
          ...
          {forgotSent ? (
            <div className="text-center py-4 space-y-4">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Jika email <span className="text-white">{forgotEmail}</span> terdaftar,
                  kamu akan menerima link reset password dalam beberapa menit.
                </p>
              </div>
              <p className="text-slate-500 text-xs">Cek folder spam jika tidak ada di inbox.</p>
            ...
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={forgotEmail} onChange={e => { setForgotEmail(e.target.value); setForgotError('') }}
                    ...
                    className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-800/60 border border-slate-600/50 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500/50 transition-all"
                  />
                </div>
              </div>
              ...
              <button type="button" onClick={() => { setShowForgot(false); setForgotError('') }}
                className="w-full py-3 rounded-xl text-sm font-medium text-slate-400 border border-slate-700/50 hover:text-white hover:border-slate-600 transition-colors">
                Kembali ke Login
              </button>
            </form>
          )}
    ...
    <div className="w-full">
      ...
      <div className="rounded-2xl p-8 border border-slate-700/50 shadow-2xl"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(v => !v)}
                className="absolute inset-y-0 right-3.5 flex items-center text-slate-500 hover:text-slate-300 transition-colors" tabIndex={-1}>
                {showPassword
        ...
        <button type="button" onClick={handleGoogleLogin} disabled={loading || loadingGoogle}
          ...
          {loadingGoogle
            ? <><svg className="animate-spin w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Menghubungkan...</>
